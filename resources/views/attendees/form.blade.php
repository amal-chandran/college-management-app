...
<div class="form-group {{ $errors->has('attendance_id') ? 'has-error' : '' }}">
    ...
    <div class="col-md-10">
        {!! Form::select('attendance_id',$attendances,null, ['class' => 'form-control', 'placeholder' => 'Select
        attendance', ]) !!}
    ...
    <div class="col-md-10">
        <div class="radio">
            <label for='status_present'>
                {!! Form::radio('status', 'present', (old('status', isset($attendee->status) ? $attendee->status : null)
                == 'present' ? true : null) , ['id' => 'status_present', 'class' => '', ]) !!}
                Present
            </label>
        </div>
        <div class="radio">
            <label for='status_absent'>
                {!! Form::radio('status', 'absent', (old('status', isset($attendee->status) ? $attendee->status : null)
                == 'absent' ? true : null) , ['id' => 'status_absent', 'class' => '', ]) !!}
                Absent
            </label>
        </div>
